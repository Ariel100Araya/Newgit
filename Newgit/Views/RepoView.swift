...
struct RepoView: View {
    ...
    // Currently selected issue in the issues sheet
        ...
        .sheet(isPresented: $showIssuesSheet) {
            VStack(alignment: .leading, spacing: 12) {
                ...
                if isLoadingIssues {
                    ...
                    // Two-column layout: selectable list on the left, details on the right
                    HStack(spacing: 12) {
                        List(issues, id: \.number) { issue in
                            Button(action: { self.selectedIssue = issue }) {
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            if let sel = selectedIssue {
                                Text("#\(sel.number) — \(sel.title)")
                                    .font(.headline)
                                Text(sel.state.capitalized)
                                    .font(.subheadline)
                                    .foregroundColor(sel.state == "open" ? .green : .red)
                                
                                ScrollView {
                                    if let body = sel.body, !body.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                                        Text(body)
                                            .padding(.top, 6)
                                            .frame(maxWidth: .infinity, alignment: .leading)
                                            .font(.body)
                                ...
                                HStack {
                                    ...
                                    Button(sel.state == "open" ? "Mark as done" : "Reopen") {
                                        if sel.state == "open" { closeIssue(sel) } else { reopenIssue(sel) }
                                    }
                ...
                HStack {
                    Spacer()
                    Button("Close") { showIssuesSheet = false }
                    .keyboardShortcut(.defaultAction)
                }
                .padding(.top)
            }
    ...
    private func loadIssues() {
        // Request the issue body so we can show the description in the details pane
        ...
        DispatchQueue.main.async {
            ...
            do {
                if let arr = try JSONSerialization.jsonObject(with: data) as? [[String: Any]] {
                    ...
                    // Pre-select the first issue for convenience
    ...
    private func closeIssue(_ issue: Issue) {
        DispatchQueue.global(qos: .userInitiated).async {
            DispatchQueue.main.async { self.isLoadingIssues = true }
            let args = ["issue", "close", "\(issue.number)"]
            print("RepoView.closeIssue: running gh \(args) in \(projectDirectory)")
            let res = runGHCommand(args, currentDirectory: projectDirectory)
            print("RepoView.closeIssue: exit=\(res.status) output=\(res.output)")
            DispatchQueue.main.async {
                self.isLoadingIssues = false
                if res.status == 0 {
    ...
    private func reopenIssue(_ issue: Issue) {
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            print("RepoView.reopenIssue: running gh \(args) in \(projectDirectory)")
            let res = runGHCommand(args, currentDirectory: projectDirectory)
            print("RepoView.reopenIssue: exit=\(res.status) output=\(res.output)")
            DispatchQueue.main.async {
                self.isLoadingIssues = false
                if res.status == 0 {
...
// `Issue` model is provided by `Newgit/Model/IssueModels.swift` and shared across the app.
// The local duplicate was removed to avoid redeclaration errors.
